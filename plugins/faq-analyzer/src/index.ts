...
async function isTableCreated(tableClient: bp.Client, botId: string, logger: Logger): Promise<boolean> {
  try {
    ...
    const state = rawState && 'payload' in rawState ? (rawState as { payload: TableState.Table['payload'] }) : undefined
...
async function setBotTableState(tableClient: bp.Client, botId: string, logger: Logger): Promise<void> {
  try {
...
plugin.on.beforeIncomingMessage('*', async (props) => {
  try {
    ...
    const tableAlreadyCreated = await isTableCreated(tableClient, props.ctx.botId, props.logger)

    if (tableAlreadyCreated) {
      props.logger.debug(`Table "${tableName}" already created, skipping creation`)
      return undefined
    }

    props.logger.info(`Creating table "${tableName}" with schema ${JSON.stringify(schema)}`)

    await safeCreateTableAndSetState(tableClient, tableName, props.ctx.botId, props.logger)
  } catch (err) {
    if (err instanceof Error) {
...
async function expandFollowUpQuestion(
  ...
  const contextualQuestion = await zai.extract(
    ...
    z.object({
      standaloneQuestion: z.string().describe('The follow-up question rewritten as a complete standalone question'),
    }),
    {
...
async function getMostSimilarQuestion(
  ...
  const mostSimilarQuestionResult = await zai.extract(
    ...
    z.union([z.object({ mostSimilarQuestion: z.string() }), z.array(z.object({ mostSimilarQuestion: z.string() }))]),
    {
...
async function markConversationAsAnalyzed(
  ...
  try {
    ...
    try {
      ...
      currentState =
        rawState && 'payload' in rawState ? (rawState as { payload: FaqAnalyzedState['payload'] }) : undefined
    } catch {
    ...
    if (!eventCreatedAt || !currentTs || new Date(eventCreatedAt).getTime() > new Date(currentTs).getTime()) {
      await tableClient.setState({
        type: 'conversation',
        id: conversationId,
        name: 'faqAnalyzed',
...
plugin.on.afterIncomingMessage('*', async (props) => {
  ...
  const eventCreatedAt = (props as any).event?.createdAt ?? (props as any).event?.createdOn
  ...
  if (lastProcessedAt && eventCreatedAt) {
    ...
    if (eventTs <= lastTs) {
      props.logger.debug(`Skipping message already processed. eventTs=${eventTs} lastTs=${lastTs}`)
      return
    }
  }
  ...
  const limitedMessages = eventCreatedAt
    ? messages.filter((m) => new Date(m.createdAt).getTime() <= new Date(eventCreatedAt).getTime())
    : messages
  ...
  if (alreadyProcessed?.payload?.done) {
    ...
    await markConversationAsAnalyzed(tableClient, props.data.conversationId, props.logger, eventCreatedAt)
    return
  }
  ...
  try {
    ...
    await extractAndProcessQuestions(props, tableClient, tableName, fullUserMessages, userMessages, questionSchema)
...
async function handleExactMatch(
  ...
  props.logger.info(`Incremented count for exact question: "${existingRecord.question}" to ${currentCount + 1}`)
  return true
}

function isLikelyEntityChange(existingQuestions: string[], normalizedQuestion: string, props: TableProps): boolean {
...
async function checkSimilarity(
  ...
  return await zai.check({ newQuestion: normalizedQuestion, existingQuestions }, ZAI_CHECK_SIMILARITY_INSTRUCTIONS)
}
...
async function processExistingRecords(
  ...
  const exactMatch = existingRecords.find((r: QuestionRecord) => r.question.trim().toLowerCase() === normalizedQuestion)
  ...
  const zai = new Zai({ client: tableClient })
  const isSimilarToExisting = await checkSimilarity(zai, normalizedQuestion, existingQuestions, props)

  if (!isSimilarToExisting) {
    return false
  }

  const mostSimilarQuestion = await getMostSimilarQuestion(zai, normalizedQuestion, existingQuestions)
  ...
  const existingRecord = existingRecords.find((r: QuestionRecord) => r.question === mostSimilarQuestion)

  if (!existingRecord) {
    return false
  }

  return await confirmAndUpdateSimilarQuestion(zai, tableClient, tableName, normalizedQuestion, existingRecord, props)
}

async function processQuestion(props: TableProps, tableClient: bp.Client, tableName: string, questionText: string) {
  const normalizedQuestion = questionText.trim().toLowerCase().replace(/\?+$/, '').trim()
