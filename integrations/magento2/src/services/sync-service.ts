...
async function ensureTableSchema(state: SyncState, httpHeaders: Record<string, string>, log: bp.Logger): Promise<void> {
...
export async function setupInitialSync(
  ...
  if (input.filters_json) {
    filterCriteria = await processFilters(input.filters_json, attributeMappings, config, log)
  }
...
export async function processFirstPage(
  ...
  const { products, totalCount } = await fetchProducts(1, PAGE_SIZE, state.filterCriteria, config, log)

  log.info(`Total products to sync: ${totalCount}`)

  if (products.length === 0) {
    log.warn('No products found to sync.')
    return { totalCount: 0, processedCount: 0 }
  }
...
export async function processRemainingPages(
  ...
  while (true) {
    const { products } = await fetchProducts(currentPage, PAGE_SIZE, state.filterCriteria, config, log)
