...
import { TableSchema, TableResponse, TablesListResponse } from '../types/botpress'
import { AttributeMapping, ColumnNameMapping, OAuthClient, OAuthToken, ProductRow } from '../types/magento'
...
export async function createOrGetTable(
  ...
  const listTablesResponse = await apiCallWithRetry(() => axios.get<TablesListResponse>(apiBaseUrl, { headers: httpHeaders }), log)
  const existingTables = listTablesResponse.data.tables || []
  let foundTable = existingTables.find((t: { id: string; name: string }) => t.name === tableName)
  ...
  if (!foundTable) {
    ...
    if (Object.keys(defaultProperties).length > 20) {
      throw new Error(`Too many columns: ${Object.keys(defaultProperties).length}. Max is 20.`)
    }
    
    const createTablePayload = { name: tableName, schema: { type: 'object', properties: defaultProperties } }
    const createTableResponse = await apiCallWithRetry(() => axios.post<TableResponse>(apiBaseUrl, createTablePayload, { headers: httpHeaders }), log)
    ...
    const tableDetailsResponse = await apiCallWithRetry(() => axios.get<TableResponse>(`${apiBaseUrl}/${tableId}`, { headers: httpHeaders }), log)
