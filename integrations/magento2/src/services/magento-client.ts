...
export class MagentoClient {
   ...
   */
  async makeRequest<T = any>(endpoint: string, method: string = 'GET', data?: ApiRequestData): Promise<T> {
    ...
    for (let i = 0; i < maxRetries; i++) {
      try {
        ...
        this.logger.forBot().info(`API response received - Status: ${response.status}`)
        ...
        if (axios.isAxiosError(error)) {
          ...
          this.logger.forBot().error(`Response data: ${JSON.stringify(error.response?.data)}`)
        }
  ...
  async getProducts(filterCriteria?: string): Promise<ProductSearchResponse> {
    const endpoint = filterCriteria ? `/V1/products?${filterCriteria}` : '/V1/products'
   ...
   */
  async getAttributeOptions(attributeCode: string): Promise<AttributeOptionsApiResponse> {
  ...
  validateResponse(data: any) {
    try {
      ...
      this.logger.forBot().info(`Successfully validated ${parsed.items?.length || 0} products`)
  ...
  async sendWebhook(webhookId: string, payload: WebhookPayload): Promise<void> {
    ...
    try {
      ...
      if (response.status !== 200) {
        this.logger.forBot().error(`Webhook failed with status ${response.status}`)
      }
    } catch (error) {
      this.logger.forBot().error('Failed to send webhook to continue sync:', error)
    }
  }
  ...
  async searchProducts(request: ProductSearchRequest): Promise<ProductSearchResponse> {
    return this.makeRequest<ProductSearchResponse>('/V1/products', 'GET', request)
  }

  /**
   * Updates stock item with proper typing
   */
  async updateStockItem(sku: string, request: StockItemRequest): Promise<StockItemResponse> {
    return this.makeRequest<StockItemResponse>(`/V1/products/${sku}/stockItems/${sku}`, 'PUT', request)
  }
}
 ...
 */
export function createMagentoClient(config: MagentoConfiguration, logger: bp.Logger): MagentoClient {
