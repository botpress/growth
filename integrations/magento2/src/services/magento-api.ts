...
import { MagentoConfiguration, Filter, AttributeMapping, OAuthClient, OAuthToken, MagentoProduct } from '../types/magento'
...
export function createHeaders(config: MagentoConfiguration, botId: string): Record<string, string> {
  return {
...
export function createHttpHeaders(config: MagentoConfiguration, botId: string): Record<string, string> {
  return {
...
function createFilterGroup(filters: Filter[], groupIndex: number): string[] {
  return filters.map((filter: Filter, filterIndex: number) => {
    ...
    if (filter.value !== undefined && filter.value !== null && filter.condition !== 'notnull' && filter.condition !== 'null') {
...
export function buildFilterCriteria(filters: Filter[], attributeMappings: AttributeMapping): string {
  ...
  try {
    ...
    const attrResponse = await apiCallWithRetry(
      () => axios({ 
        method: 'GET', 
        url: attrUrl, 
        headers: { ...oauth.toHeader(oauth.authorize({ url: attrUrl, method: 'GET' }, token)), ...headers } 
      }), 
      log
    )
...
export async function processFilters(
  ...
  try {
    ...
    const attributeFields = Array.from(new Set(filters.map((f: Filter) => f.field).filter((f: string | undefined): f is string => f !== undefined && !standardFields.includes(f))))

    // Fetch attribute mappings for custom fields
    for (const attrCode of attributeFields) {
      const options = await fetchAttributeOptions(attrCode, magentoDomain, storeCode, oauth, token, headers, log)
      if (options) {
    ...
    filters = filters.map((filter: Filter) => {
      if (filter.field !== undefined && filter.value !== undefined && attributeMappings[filter.field]?.[filter.value.toString()] !== undefined) {
        return { ...filter, value: attributeMappings[filter.field]?.[filter.value.toString()] }
      }
      return filter
    })
...
export async function fetchProducts(
  ...
  return { products: parsed.items as MagentoProduct[], totalCount: parsed.total_count }
}
