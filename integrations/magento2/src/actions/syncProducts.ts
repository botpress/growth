...
export const syncProducts: bp.IntegrationProps["actions"]["syncProducts"] =
  async ({ ctx, input, logger, client }) => {
    ...
    if (typeof _attributeMappings === "string") {
      ...
      typeof _attributeMappings === "object" &&
    ...
    try {
      ...
      if (isInitialRun) {
        ...
        log.info(
          `Custom attributes to sync: ${customAttributeCodes.join(", ")}`,
        );
          ...
          try {
            ...
            const attributeFields = Array.from(
              new Set(
                filters
                  .map((f: any) => f.field)
                  .filter((f: string) => f && !standardFields.includes(f)),
              ),
            );
            ...
            for (const attrCode of attributeFields) {
              try {
                ...
                const attribute = ProductAttributeSchema.parse(
                  attrResponse.data,
                );
                ...
                log.warn(
                  `Failed to get attribute mapping for ${attrCode}:`,
                  error,
                );
              }
            }
            ...
            filters = filters.map((filter: any) => {
              if (
                ...
                return {
                  ...filter,
                  value: attributeMappings[filter.field]?.[filter.value],
                };
              }
              return filter;
            });
            ...
            filters.forEach((filter: any, idx: number) => {
              ...
              if (
                ...
                filterGroups.push(
                  `${filterGroup}&searchCriteria[filterGroups][${idx}][filters][0][value]=${encodeURIComponent(filter.value)}`,
                );
              } else {
        ...
        if (!foundTable) {
          ...
          const createTableResponse = await apiCallWithRetry(
            () =>
              axios.post(apiBaseUrl, createTablePayload, {
                headers: httpHeaders,
              }),
            log,
          );
          ...
          for (const attrCode of customAttributeCodes) {
            try {
              ...
              if (attribute.options && attribute.options.length > 0) {
                ...
                log.info(
                  `Mapped ${attribute.options.length} options for attribute ${attrCode}`,
                );
              }
            } catch (error) {
      ...
      while (hasMorePages) {
        if (Date.now() - startTime >= MAX_EXECUTION_TIME_MS) {
          ...
          log.info(
            `Execution time limit approaching after ${elapsedNow}ms. Triggering webhook to continue sync on next run.`,
          );
          break;
        }
        ...
        const remainingProductsInPage =
          products.length - currentPageProductIndex;
        log.info(
        ...
        if (!tableSchema) {
          const tableDetailsResponse = await apiCallWithRetry(
            () =>
              axios.get(`${apiBaseUrl}/${tableId}`, { headers: httpHeaders }),
            log,
          );
        ...
        const rowsToInsert = await processProducts(
          products.slice(currentPageProductIndex),
          {
            ...
            onTimeLimit: () => {
              log.info(
                `Time limit reached while processing products. Will resume from current position.`,
              );
              return true;
            },
          },
        ...
        if (rowsToInsert.length > 0) {
          log.info(
            `Inserting ${rowsToInsert.length} rows into table ${table_name}`,
          );
          await apiCallWithRetry(
            () =>
              axios.post(
          ...
          log.debug(
            `Elapsed time so far: ${elapsed}ms (remaining: ${MAX_EXECUTION_TIME_MS - elapsed}ms)`,
          );
        }
        ...
        const currentSyncedCount =
          (currentPage - 1) * pageSize + currentPageProductIndex;
        hasMorePages = currentSyncedCount < totalCount;

        if (Date.now() - startTime >= MAX_EXECUTION_TIME_MS) {
          log.info(
            `Time limit reached after processing page ${currentPage} (product index: ${currentPageProductIndex}). Breaking to trigger webhook.`,
          );
          break;
        }
      }
      ...
      if (hasMorePages) {
        log.info(
          "Time limit reached. Creating magentoSyncContinue event for next batch.",
        );
        try {
          await client.createEvent({
            ...
            payload: {
              ...
              _attributeMappings:
                typeof _attributeMappings === "string"
                  ? _attributeMappings
          ...
          log.info(
            "Successfully created magentoSyncContinue event for continued sync.",
          );
        } catch (err: any) {
          log.error(
            "Failed to create magentoSyncContinue event for continued sync.",
            err,
          );
        }
        return {
      ...
      log.info(
        `Sync completed successfully in ${totalElapsed}ms. Total products synced: ${totalCount}`,
      );
      return {
        success: true,
...
async function processProducts(
  ...
  for (const product of products) {
    if (Date.now() - startTime >= MAX_EXECUTION_TIME_MS) {
      logger.warn(
        `Time limit reached while processing products. Stopping at product ${product.sku}.`,
      );
      if (onTimeLimit()) {
        break;
      }
    ...
    try {
      ...
      if (product.extension_attributes?.stock_item) {
        ...
        isInStock =
          product.extension_attributes.stock_item.is_in_stock || false;
      } else {
        ...
        if (
          ...
          if (attr) {
            ...
            if (
              ...
              attributeMappings[attr.attribute_code]?.[attr.value] !== undefined
            ) {
              value = attributeMappings[attr.attribute_code]?.[attr.value];
            } else if (
              Array.isArray(attr.value) &&
              attributeMappings[attr.attribute_code]
            ) {
              value = attr.value
                .map(
                  (v: any) => attributeMappings[attr.attribute_code]?.[v] ?? v,
                )
                .join(", ");
            }
          }
