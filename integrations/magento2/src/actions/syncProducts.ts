  ...
  try {
    ...
    if (isInitialRun) {
      ...
      if (filters_json) {
        try {
          ...
          const attributeFields = Array.from(
            new Set(filters.map((f: any) => f.field).filter((f: string) => f && !standardFields.includes(f)))
          )
      ...
      if (!foundTable) {
        ...
        const createTableResponse = await apiCallWithRetry(
          () => axios.post(apiBaseUrl, createTablePayload, { headers: httpHeaders }),
          log
        )
      ...
      await apiCallWithRetry(
        () => axios.post(`${apiBaseUrl}/${tableId}/rows/delete`, { deleteAllRows: true }, { headers: httpHeaders }),
        log
      )
    ...
    while (hasMorePages) {
      ...
      if (rowsToInsert.length > 0) {
        ...
        await apiCallWithRetry(
          () => axios.post(`${apiBaseUrl}/${tableId}/rows`, { rows: rowsToInsert }, { headers: httpHeaders }),
          log
        )
      ...
      try {
        ...
        const payload = {
          ...
          data: {
            ...
            _attributeMappings:
              typeof _attributeMappings === 'string' ? _attributeMappings : JSON.stringify(attributeMappings),
