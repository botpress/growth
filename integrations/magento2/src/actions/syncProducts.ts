...
import { BotpressContext, SyncInput, SyncResult, SyncContext, SyncState, ColumnNameMapping } from '../types/magento'
import { parseAttributeMappings, parseColumnNameMappings } from '../utils/magento'
import { createOAuthClient, createHeaders, createHttpHeaders, sendWebhook } from '../services/magento-api'
...
function createSyncContext(ctx: BotpressContext): SyncContext {
  return {
    config: { ...ctx.configuration, store_code: ctx.configuration.store_code || 'all' },
    oauth: createOAuthClient(ctx.configuration),
    token: { key: ctx.configuration.access_token, secret: ctx.configuration.access_token_secret },
...
function createWebhookPayload(input: SyncInput, state: SyncState, runId: string, totalCount: number) {
  return {
    ...
    data: {
      ...
      _attributeMappings: typeof input._attributeMappings === 'string' ? input._attributeMappings : JSON.stringify(state.attributeMappings),
...
function recreateColumnNameMappings(input: SyncInput): ColumnNameMapping {
  const parsedColumnNameMappings = parseColumnNameMappings(input._columnNameMappings)
  
  if (Object.keys(parsedColumnNameMappings).length === 0 && input._customAttributeCodes && input._customAttributeCodes.length > 0) {
...
async function handleInitialRun(
  ...
  const state = await setupInitialSync(input, context.config, context.oauth, context.token, context.headers, context.httpHeaders, context.apiBaseUrl, log)
  const { rowsToInsert, totalCount, processedCount } = await processFirstPage(state, input, context.config, context.oauth, context.token, context.headers, context.httpHeaders, context.apiBaseUrl, log)
...
async function handleContinuation(
  ...
  const { totalProcessed } = await processRemainingPages(state, input, context.config, context.oauth, context.token, context.headers, context.httpHeaders, context.apiBaseUrl, log)
