...
export const getProducts: bp.IntegrationProps["actions"]["getProducts"] =
  async ({ ctx, input, logger }) => {
    ...
    if (typeof filtersInput === "string") {
      ...
      try {
        ...
        logger
          .forBot()
          .info(
            `Successfully parsed JSON input: ${JSON.stringify(filtersInput)}`,
          );
      } catch (err) {
    ...
    try {
      ...
      if (Array.isArray(filters)) {
        logger
          .forBot()
          .info(`Building filter criteria for ${filters.length} filters`);
        ...
        filters.forEach(
          (
            ...
            logger
              .forBot()
              .info(`Processing filter ${idx + 1}: ${JSON.stringify(filter)}`);
            if (!filter.field || !filter.condition) {
              logger
                .forBot()
                .warn(
                  `Skipping filter ${idx + 1} - missing field or condition`,
                );
              return;
            }
            ...
            if (
              ...
              logger
                .forBot()
                .info(`Added filter group ${idx + 1}: ${fullFilterGroup}`);
            } else {
              filterGroups.push(filterGroup);
              logger
                .forBot()
                .info(
                  `Added filter group ${idx + 1} (no value): ${filterGroup}`,
                );
            }
          },
        );
        if (filterGroups.length > 0) {
          ...
          logger
            .forBot()
            .info(`Built filter criteria with ${filterGroups.length} groups`);
        } else {
    ...
    const url = `https://${magento_domain}/rest/${store_code}/V1/products?${filterCriteria}`;
    ...
    try {
      ...
      if (response.data && typeof response.data === "object") {
        ...
        if (response.data.items) {
          logger
            .forBot()
            .info(`Found ${response.data.items.length} products in response`);
        }
      }
      ...
      try {
        ...
        logger
          .forBot()
          .error(`Raw response data: ${JSON.stringify(response.data)}`);
        return {
          result: { items: [], search_criteria: {}, total_count: 0 },
