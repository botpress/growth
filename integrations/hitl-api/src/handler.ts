 ...
 */
export const handleAgentMessage = async (client: any, body: z.infer<typeof AgentMessagePayload>) => {
 ...
 */
export const handleAgentAssigned = async (client: any, body: z.infer<typeof AgentAssignedPayload>) => {
  const { conversation } = await client.getOrCreateConversation({
    channel: 'hitl',
    tags: {
      externalId: body.remoteConversationId,
 ...
 */
export const handleStopHitl = async (client: any, body: z.infer<typeof StopHitlPayload>) => {
  const { conversation } = await client.getOrCreateConversation({
    channel: 'hitl',
    tags: {
      externalId: body.remoteConversationId,
 ...
 */
export const handler = async ({ req, client, logger }: { req: any; client: any; logger: any }) => {
  if (!req.body) {
