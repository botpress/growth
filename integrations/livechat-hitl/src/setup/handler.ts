...
import { liveChatWebhookPayloadSchema, LiveChatTransferred } from 'src/definitions/livechatEvents'
...
export const handler: bp.IntegrationProps['handler'] = async ({ ctx, req, logger, client }) => {
  ...
  switch (action) {
    ...
    case 'chat_deactivated': {
      ...
      await handleChatDeactivated(webhookPayload, logger, client, conversation as bp.AnyMessageProps['conversation'])
      break
    }
    ...
    case 'chat_transferred': {
      ...
      await handleChatTransferred(webhookPayload, logger, client, conversation as bp.AnyMessageProps['conversation'])
      break
    }
