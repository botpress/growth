...
export class LiveChatApi {
  ...
  public async createCustomerToken(redirectUri?: string): Promise<{
    ...
    try {
      ...
      this.logger.forBot().error("Error creating customer token:", error.response?.data || error.message);
      throw error;
    }
  }
  ...
  public async getOrganizationId(licenseId: string, accessToken: string): Promise<any> {
    ...
    try {
      ...
      this.logger.forBot().error("Error fetching organization_id:", error.response?.data || error.message);
      throw error;
    }
  }
