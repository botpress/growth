...
export class LiveChatApi {
  ...
  public async deactivateChatWithAgent(
    agentToken: string,
    chatId: string,
  ): Promise<any> {
    ...
    try {
      this.logger
        .forBot()
        .info(`Deactivating chat ${chatId} with agent token...`);
      const response = await axios.post(url, payload, {
        headers: {
  ...
  public async startAgentChat(
    ...
    try {
      ...
      const chatId =
        response.data.chat_id || (response.data.chat && response.data.chat.id);
  ...
  public async getCustomer(
    ...
    validateRequiredParams(
      { customerAccessToken, organizationId },
      "getCustomer",
    );
  ...
  public async addCustomerToChat(
    ...
    validateRequiredParams(
      { agentToken, chatId, customerAccessToken },
      "addCustomerToChat",
    );

    this.logger
      .forBot()
      .debug(`addCustomerToChat - organizationId: ${this.organizationId}`);

    const customerResult = await this.getCustomer(
      customerAccessToken,
      this.organizationId,
    );
    if (!customerResult.success) {
      this.logger
        .forBot()
        .error(`Failed to get customer: ${customerResult.message}`);
      return {
        success: false,
    ...
    try {
      this.logger
        .forBot()
        .info(`Adding customer ${customerId} to chat ${chatId}...`);
      const response = await axios.post(url, payload, {
        headers: {
          Authorization: `Basic ${agentToken}`,
          "Content-Type": "application/json",
  ...
  public async validateGroup(
    ...
    try {
      ...
      this.logger
        .forBot()
        .debug(
          `Group ${groupId} validation successful. Agents: ${targetGroup.agents.length}`,
        );
  ...
  public async transferChat(
    ...
    validateRequiredParams(
      { agentToken, chatId, targetGroupId },
      "transferChat",
    );
    ...
    try {
      this.logger
        .forBot()
        .info(`Transferring chat ${chatId} to group ${targetGroupId}...`);
      ...
      this.logger
        .forBot()
        .info(
          `Chat ${chatId} transferred successfully to group ${targetGroupId}`,
        );
      return { success: true, data: response.data };
    } catch (error: any) {
      const errorData = error.response?.data || error.message;
      ...
      if (
        ...
        this.logger
          .forBot()
          .error(
            `Group ${targetGroupId} validation failed: No available agents or invalid group configuration`,
          );
        return {
          success: false,
