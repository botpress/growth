...
export const register: RegisterFunction = async ({ ctx, logger }) => {
  ...
  if (!ctx.configuration.accessToken) {
    throw new RuntimeError('Configuration is incomplete. Please provide an Access Token.')
  }
  ...
  try {
    ...
    logger.forBot().info('Successfully listed admins. Integration credentials are valid.')
  } catch (error: any) {
    logger.forBot().error('Failed to list admins:', error.message || error)
    throw new RuntimeError('Failed to list admins: ' + (error.message || error))
  }
}
