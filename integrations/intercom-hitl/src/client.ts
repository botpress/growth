...
export class IntercomApi {
  ...
  constructor(accessToken: string, logger: bp.Logger) {
  ...
  public async createConversation(
    ...
    this.logger
      .forBot()
      .info(
        `Creating Intercom conversation with contact ID: ${intercomContactId} for email: ${email}`,
      );
    ...
    const conversationData = {
      ...
      body: "Botpress HITL Started for " + email,
    };
  ...
  public async closeConversation(
    ...
    this.logger
      .forBot()
      .info(`Getting admins to close conversation: ${conversationId}`);
    ...
    const admins =
      adminsResponse.data?.admins || adminsResponse.data?.data?.admins;
    ...
    this.logger
      .forBot()
      .info(
        `Using admin ID ${adminId} to close conversation: ${conversationId}`,
      );
  ...
  public async replyToConversation(
    ...
    validateRequiredParams(
      { conversationId, intercomUserId, body },
      "replyToConversation",
    );
  ...
  private async makeRequest(
    ...
    try {
      this.logger
        .forBot()
        .info(`Making ${method} request to Intercom: ${endpoint}`);
      ...
      this.logger
        .forBot()
        .error(
          `Intercom API error for ${method} ${endpoint}:`,
...
export const getClient = (accessToken: string, logger: bp.Logger) => {
