...
import { findUserOutputSchema } from '../misc/custom-schemas'
...
export const findUser: bp.IntegrationProps['actions']['findUser'] = async ({ ctx, input, logger }) => {
  try {
    ...
    const response = await jiraClient.findUser(input.accountId)
    logger.forBot().info(`Successful - Find User - ${response?.displayName || 'Unknown'} - with ID: ${input.accountId}`)
