...
export const SFMessagingConfigSchema = z.object({
  ...
  stopHITLEscalationKeywords: z
    .array(z.string())
    .default(['stop-hitl'])
    .describe('Keywords that will stop the escalation before the agent is assigned'),
  conversationNotAssignedMessage: z
    .string()
    .default("No agent assigned yet, type 'stop-hitl' to stop the escalation and get back to the bot.")
