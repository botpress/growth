...
describe('HubSpot Client Rate Limiting Infrastructure', () => {
  ...
  beforeEach(() => {
    ...
    hubspotClient = new HubSpotApi(mockContext, mockClient, 'refresh-token', 'client-id', 'client-secret', mockLogger)
  })
  ...
  describe('Rate Limiting Infrastructure Verification', () => {
    ...
    it('should validate error response structure', async () => {
      ...
      await expect(hubspotClient.getThreadInfo('test-thread-id')).rejects.toThrow(/Failed to fetch thread info:/)
