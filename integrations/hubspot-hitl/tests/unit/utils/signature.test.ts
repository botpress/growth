...
describe('validateHubSpotInboxSignature', () => {
  ...
  it('should return false for missing client secret', () => {
    ...
    const result = validateHubSpotInboxSignature(requestBody, signature, timestamp, method, webhookUrl, '', mockLogger)
  ...
  it('should return false for expired timestamp', () => {
    ...
    expect(result).toBe(false)
    expect(mockLogger.forBot().error).toHaveBeenCalledWith('Timestamp is too old:', expect.any(Number), 'ms')
  })
