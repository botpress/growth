...
export const handleOperatorAssignedUpdate = async ({
  ...
  let threadInfo: ThreadInfo = await hubSpotClient.getThreadInfo(
  ...
  if (!email && !phoneNumber) {
    logger
      .forBot()
      .error(`No email or phone number found for contact:`, contactId);
    return;
  }
  ...
  if (email) {
    ...
    if (emailUser.tags?.contactType) {
      logger
        .forBot()
        .info(
          `Found existing user with email (contactType: ${emailUser.tags.contactType})`,
        );
      user = emailUser;
    } else {
      logger
        .forBot()
        .info(`Created new user with email, trying phone number next`);
      ...
      if (phoneNumber) {
        logger
          .forBot()
          .info(`Trying to find/create user with phone number: ${phoneNumber}`);
        ...
        if (phoneUser.tags?.contactType) {
          logger
            .forBot()
            .info(
              `Found existing user with phone number (contactType: ${phoneUser.tags.contactType})`,
            );
          user = phoneUser;
        } else {
          logger
            .forBot()
            .info(
              `Created new user with phone number - no existing users found`,
            );
    ...
    logger
      .forBot()
      .info(`No email available, trying phone number: ${phoneNumber}`);
    const { user: phoneUser } = await client.getOrCreateUser({
      tags: { phoneNumber },
    });
    if (phoneUser.tags?.contactType) {
      ...
      logger
        .forBot()
        .info(`Created new user with phone number - no existing users found`);
      phoneUserToDelete = phoneUser;
    }
  }
  ...
  if (emailUserToDelete) {
    try {
      ...
      logger
        .forBot()
        .info(`Deleted unnecessary email user: ${emailUserToDelete.id}`);
    } catch (error) {
      logger
        .forBot()
        .warn(
          `Failed to delete unnecessary email user: ${emailUserToDelete.id}`,
          error,
        );
    }
  }
  ...
  if (phoneUserToDelete) {
    try {
      ...
      logger
        .forBot()
        .info(`Deleted unnecessary phone user: ${phoneUserToDelete.id}`);
    } catch (error) {
      logger
        .forBot()
        .warn(
          `Failed to delete unnecessary phone user: ${phoneUserToDelete.id}`,
          error,
        );
    }
  }
