...
export const register: RegisterFunction = async ({ ctx, client, logger }) => {
  try {
    ...
    logger
      .forBot()
      .info("Successfully accessed GoHighLevel: Integration can proceed");

    logger
      .forBot()
      .info(`Successfully retrieved ${JSON.stringify(result)} contact`);
  } catch (error) {
    logger
      .forBot()
      .error("Failed to access GoHighLevel: Check configuration", error);
