...
export const updateContact: Implementation["actions"]["updateContact"] =
  async ({ ctx, client, logger, input }) => {
    ...
    try {
      const result = await ghlClient.updateContact(
      ...
      logger
        .forBot()
        .info(
          `Successful - Update Contact - ${JSON.stringify(validatedInput)}`,
        );
      ...
      logger
        .forBot()
        .error(`'Update Contact' exception: ${JSON.stringify(errorMessage)}`);
