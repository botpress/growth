...
export const listOrders: Implementation["actions"]["listOrders"] = async ({
  ctx,
  client,
  logger,
  input,
  ...
  try {
    ...
    logger
      .forBot()
      .info(`Successful - List Orders - ${JSON.stringify(validatedInput)}`);
    ...
    logger
      .forBot()
      .error(`'List Orders' exception: ${JSON.stringify(errorMessage)}`);
