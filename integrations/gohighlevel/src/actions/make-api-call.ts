...
export const makeApiCall: Implementation["actions"]["makeApiCall"] = async ({
  ctx,
  client,
  logger,
  input,
  ...
  try {
    ...
    logger
      .forBot()
      .info(`Successful - Make API Call - ${JSON.stringify(validatedInput)}`);
    ...
    logger
      .forBot()
      .error(`'Make API Call' exception: ${JSON.stringify(errorMessage)}`);
