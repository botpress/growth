...
export const deleteContact: Implementation["actions"]["deleteContact"] =
  async ({ ctx, client, logger, input }) => {
    ...
    try {
      ...
      logger
        .forBot()
        .info(
          `Successful - Delete Contact - ${JSON.stringify(validatedInput)}`,
        );
      ...
      logger
        .forBot()
        .error(`'Delete Contact' exception: ${JSON.stringify(errorMessage)}`);
