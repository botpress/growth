...
export const getContact: Implementation["actions"]["getContact"] = async ({
  ctx,
  client,
  logger,
  input,
  ...
  try {
    ...
    logger
      .forBot()
      .info(`Successful - Get Contact - ${JSON.stringify(validatedInput)}`);
    ...
    logger
      .forBot()
      .error(`'Get Contact' exception: ${JSON.stringify(errorMessage)}`);
