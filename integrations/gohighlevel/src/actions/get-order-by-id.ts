...
export const getOrderById: Implementation["actions"]["getOrderById"] = async ({
  ctx,
  client,
  logger,
  input,
  ...
  try {
    const result = await ghlClient.getOrderById(
    ...
    logger
      .forBot()
      .info(`Successful - Get Order By Id - ${JSON.stringify(validatedInput)}`);
    ...
    logger
      .forBot()
      .error(`'Get Order By Id' exception: ${JSON.stringify(errorMessage)}`);
