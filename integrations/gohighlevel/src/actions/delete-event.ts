...
export const deleteEvent: Implementation["actions"]["deleteEvent"] = async ({
  ctx,
  client,
  logger,
  input,
  ...
  try {
    ...
    logger
      .forBot()
      .info(`Successful - Delete Event - ${JSON.stringify(validatedInput)}`);
    ...
    logger
      .forBot()
      .error(`'Delete Event' exception: ${JSON.stringify(errorMessage)}`);
