...
const syncKb: bp.IntegrationProps["actions"]["syncKb"] = async ({
  ...
  try {
    ...
    logger
      .forBot()
      .info("Deleting existing Google Sheets files from Knowledge Base");
    ...
    if (totalFailures > 0) {
      logger
        .forBot()
        .error(
          `Upload completed with ${totalFailures} failures out of ${uploadTasks.length} total rows`,
        );
    }

    logger
      .forBot()
      .info(`Successfully synced ${recordsProcessed} rows to Knowledge Base`);
    ...
    logger
      .forBot()
      .error("Error syncing Google Sheets to Knowledge Base", { error });
