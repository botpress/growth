...
export const startHitl: bp.IntegrationProps["actions"]["startHitl"] = async ({
  ...
  if (!userId) {
    throw new RuntimeError(
      "[StartHitl] Action requires a userId from the input.",
    );
  }
  ...
  if (!emailParts[0]) {
    logger
      .forBot()
      .error("[CreateUser] Invalid email format: no local part found.");
  ...
  logger
    .forBot()
    .info(
      `[CreateUser] Generated uniqueHitlId: ${uniqueHitlId} for email: ${email}`,
    );
  ...
  try {
    logger
      .forBot()
      .info(
        `[StartHitl] Using Botpress userId '${email}' directly as visitorId for Brevo.`,
      );
    ...
    const brevoExternalConversationResponse =
      await brevoClient.createConversation(uniqueHitlId, initialMessage);
    logger
      .forBot()
      .info(
        "Full response from brevoClient.createConversation:",
        brevoExternalConversationResponse,
      );
    ...
    logger
      .forBot()
      .info(
        `Got/Created Botpress HITL channel conversation with ID: ${conversation.id}`,
      );
    ...
    logger
      .forBot()
      .error(`'startHitl' action failed: ${errorMessage}`, error.stack);
    if (error instanceof RuntimeError) {
      throw error;
    }
    throw new RuntimeError(
      `Failed to start Brevo HITL session: ${errorMessage}`,
    );
  }
};
...
export const createUser: bp.IntegrationProps["actions"]["createUser"] = async ({
  ...
  try {
    ...
    logger
      .forBot()
      .info(
        `[CreateUser] Called getOrCreateUser. Botpress User ID: ${botpressUser.id}. Unique HITL ID for Brevo visitorId: ${email}`,
      );
    ...
    logger
      .forBot()
      .error(`[CreateUser] Action failed: ${errorMessage}`, error.stack);
    if (error instanceof RuntimeError) throw error;
    throw new RuntimeError(
      `Failed to create/map user for Brevo HITL: ${errorMessage}`,
    );
  }
};
