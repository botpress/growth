...
export const handler: bp.IntegrationProps["handler"] = async ({
  ...
  logger
    .forBot()
    .debug("Handler received request from Brevo with raw payload:", req.body);
  ...
  try {
    ...
    if (typeof req.body !== "string") {
      logger
        .forBot()
        .error("Handler received a non-string body. Type:", typeof req.body);
      return;
    }
    brevoEventPayload = JSON.parse(req.body);
    logger
      .forBot()
      .info("Conversation ID from Brevo:", brevoEventPayload.conversationId);
  } catch (error: any) {
    logger
      .forBot()
      .error(
  ...
  if (!validationResult.success) {
    logger
      .forBot()
      .error("Invalid Brevo event payload:", validationResult.error.format());
    return;
  }
  ...
  switch (eventType) {
    case "conversationFragment":
      await handleOperatorReplied({
        brevoEvent: validationResult.data,
        client,
        logger,
      });
      break;
    ...
    default:
      logger
        .forBot()
        .warn(
          `Handler: Unhandled Brevo event type: ${eventType}. Payload:`,
