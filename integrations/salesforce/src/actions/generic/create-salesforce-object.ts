...
export const createSalesforceObject = async <T extends object>(
  ...
  logger
    .forBot()
    .info(
      `Attempting to create a ${objectType} from from ${JSON.stringify(input)}`
    );
  const errorMsg = `'Create ${objectType}' error:`;

  try {
    const connection = await getConnection(client, ctx, logger);
    const response = await connection.sobject(objectType).create(input);
