...
export default new bp.Integration({
  register: async ({ ctx, logger }) => {
    logger
      .forBot()
      .info(
        `Registering SharePoint Excel integration for bot: ${ctx.botId}. Performing connection test.`,
      );
    ...
    try {
      ...
      logger
        .forBot()
        .info(
          `SharePoint connection test successful during registration. Site ID: ${siteId}`,
        );
    } catch (error: any) {
      logger
        .forBot()
        .error(
          `SharePoint connection test failed during registration: ${error.message}`,
          error.stack,
        );
      throw new sdk.RuntimeError(
        `SharePoint connection validation failed during registration: ${error.message}`,
      );
    }
  },
  ...
  unregister: async ({ ctx, logger }) => {
    logger
      .forBot()
      .info(`Unregistering SharePoint Excel integration for bot: ${ctx.botId}`);
  ...
  actions: {
    syncExcelFile: async ({ ctx, input, logger }) => {
      ...
      try {
        logger
          .forBot()
          .debug(`Fetching Excel file from URL: ${sharepointFileUrl}`);

        // If in debug mode or if file not found, list available document libraries
        let fileContentBuffer: Buffer;
        try {
          fileContentBuffer =
            await spClient.getFileContentByUrl(sharepointFileUrl);
          ...
          if (
            ...
            logger
              .forBot()
              .warn(
                "File not found. Listing available document libraries to help diagnose the issue...",
              );
            try {
              ...
              logger
                .forBot()
                .info(
                  `Available document libraries in site "${ctx.configuration.siteName}":`,
                );
              ...
              logger
                .forBot()
                .info(
                  "Please ensure your file URL matches one of these document libraries.",
                );
            } catch (listError) {
              logger
                .forBot()
                .error("Could not list document libraries:", listError);
            }
          }
          throw error;
        }
        ...
        logger
          .forBot()
          .info(
            `Excel workbook loaded with ${workbook.SheetNames.length} sheet(s): ${workbook.SheetNames.join(", ")}`,
          );
        ...
        try {
          ...
          logger
            .forBot()
            .info(`Parsed sheetTableMapping: ${JSON.stringify(sheetToTable)}`);
        } catch (err) {
          logger.forBot().error(`Failed to parse sheetTableMapping: ${err}`);
          throw new sdk.RuntimeError(
            "Invalid sheetTableMapping format. Use JSON or comma-separated pairs.",
          );
        }

        // Validate that all sheets in the mapping exist in the workbook
        const missingSheets = sheetsToProcess.filter(
        ...
        for (const currentSheetName of sheetsToProcess) {
          ...
          if (!worksheet) {
            logger
              .forBot()
              .warn(`Sheet "${currentSheetName}" is undefined, skipping`);
            continue;
          }
          ...
          logger
            .forBot()
            .info(
              `Sheet "${currentSheetName}" has ${jsonData.length} rows (including header)`,
            );

          if (jsonData.length === 0) {
            logger
              .forBot()
              .warn(`Sheet "${currentSheetName}" is empty, skipping`);
            continue;
          }
          ...
          if (!excelHeaders || excelHeaders.length === 0) {
            logger
              .forBot()
              .warn(`Sheet "${currentSheetName}" has no header row, skipping`);
            continue;
          }
          ...
          logger
            .forBot()
            .info(
              `Found ${rowsData.length} data rows in sheet "${currentSheetName}"`,
            );
          ...
          logger
            .forBot()
            .info(
              `Using mapped table name: "${tableNameForSheet}" for sheet "${currentSheetName}"`,
            );
          ...
          try {
            logger
              .forBot()
              .debug(
                `Looking for existing table named: "${tableNameForSheet}"`,
              );
            const listTablesResponse = await axios.get(apiBaseUrl, {
              headers: httpHeaders,
            });
            ...
            if (foundTable) {
              ...
              logger
                .forBot()
                .info(
                  `Table "${tableNameForSheet}" (ID: ${tableId}) found. Clearing all existing rows.`,
                );
              try {
                ...
                logger
                  .forBot()
                  .info(
                    `Successfully cleared all rows from table "${tableNameForSheet}" (ID: ${tableId}).`,
                  );
                ...
                logger
                  .forBot()
                  .warn(
                    `This may result in duplicate data if old rows weren't properly cleared.`,
                  );
              }
            }
            ...
            if (!foundTable) {
              logger
                .forBot()
                .info(
                  `Table "${tableNameForSheet}" not found. Creating it now.`,
                );
              ...
              excelHeaders.forEach((header, index) => {
                ...
                if (cleanHeader) {
                  ...
                  logger
                    .forBot()
                    .debug(
                      `Column "${cleanHeader}" detected as type: ${columnType}`,
                    );
                }
              });
              ...
              if (
                ...
                !createTableResponse.data.table ||
                !createTableResponse.data.table.id
              ) {
                logger
                  .forBot()
                  .error(
              ...
              logger
                .forBot()
                .info(
                  `Table "${tableNameForSheet}" created successfully with ID: ${tableId}.`,
                );
            }
            ...
            if (rowsData.length > 0 && tableId) {
              logger
                .forBot()
                .info(
                  `Populating table "${tableNameForSheet}" (ID: ${tableId}) with ${rowsData.length} new rows.`,
                );
              ...
              if (foundTable) {
                ...
                try {
                  const tableDetailsResponse = await axios.get(
                    `${apiBaseUrl}/${tableId}`,
                    { headers: httpHeaders },
                  );
                  ...
                  logger
                    .forBot()
                    .warn(
                      "Could not fetch table schema, will use string types for all columns",
                    );
                }
              }
                  ...
                  excelHeaders.forEach((header, index) => {
                    ...
                    if (cleanHeader) {
                      ...
                      if (
                        ...
                        if (
                          tableSchema?.properties?.[cleanHeader]?.type ===
                          "number"
                        ) {
                          const numValue = Number(value);
                          rowObject[cleanHeader] = isNaN(numValue)
                            ? value
                            : numValue;
                        } else {
              ...
              if (rowsToInsert.length > 0) {
                ...
                while (processedRows < totalRows) {
                  ...
                  logger
                    .forBot()
                    .info(
                      `Processed ${processedRows}/${totalRows} rows for table "${tableNameForSheet}"`,
                    );
                }
                ...
                logger
                  .forBot()
                  .info(
                    `Successfully populated table "${tableNameForSheet}" with ${rowsToInsert.length} rows`,
                  );
                ...
                logger
                  .forBot()
                  .warn(
                    `Data rows were present in sheet "${currentSheetName}", but no valid rows could be constructed`,
                  );
              }
            } else if (!tableId) {
              logger
                .forBot()
                .error("Table ID not available, cannot populate rows");
            } else {
              logger
                .forBot()
                .info(
                  `No data rows to populate in sheet "${currentSheetName}"`,
                );
            }
          } catch (apiError: any) {
            logger
            ...
            if (apiError.response?.data) {
              logger
                .forBot()
                .error(
                  `API Response: ${JSON.stringify(apiError.response.data)}`,
                );
            }
            ...
            if (!processAllSheets) {
              ...
              logger
                .forBot()
                .warn(
                  `Failed to process sheet "${currentSheetName}", continuing with other sheets...`,
                );
            }
          }
        ...
        logger
          .forBot()
          .info(`Processed ${processedSheets.length} sheet(s) successfully`);
        ...
        logger
          .forBot()
          .error(
            `Error during Excel file sync for "${sharepointFileUrl}": ${error.message}`,
            error.stack,
          );
        throw error;
      }
