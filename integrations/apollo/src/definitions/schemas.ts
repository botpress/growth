...
export const SearchPayloadSchema = z.object({
  q_keywords: z.string().optional().describe('Keywords to narrow search. Can include combinations of names, job titles, employers (company names), and email addresses.'),
  contact_stage_ids: z.array(z.string()).optional().describe('Apollo IDs for the contact stages that you want to include in your search results. Adding multiple IDs will return matching contacts from all of the specified stages.'),
  sort_by_field: z.string().optional().describe('Field to sort the search results by. Valid options are "contact_last_activity_date", "contact_email_last_opened_at", "contact_email_last_clicked_at", "contact_created_at" and "contact_updated_at".'),
  sort_ascending: z.boolean().optional().describe('Set true to sort the results in ascending order. Default is false. Must be used in conjunction with sortByField.'),
  page: z.number().optional().describe('Page number of results to return.'),
  per_page: z.number().optional().describe('Number of contacts to return per page. Limiting the number of contacts returned will speed up the search process.'),
})
...
export const PersonPayloadSchema = z.object({
  ...
  domain: z.string().optional().describe('The domain name for the person\'s current or previous employer. Do NOT include www., @ or similar.'),
...
export const ContactCampaignStatusSchema = z.object({
  id: z.string().describe("Unique identifier for this contact campaign status record"),
  emailer_campaign_id: z
    .string()
    .describe("ID of the email sequence (emailer campaign) this status belongs to"),
  ...
  added_at: z
    .string()
    .datetime()
    .describe("Timestamp when the contact was added to this sequence"),
  added_by_user_id: z
    .string()
    .describe("ID of the user who added this contact to the sequence"),
  finished_at: z
    .string()
    .datetime()
    .nullable().optional()
  ...
  manually_set_unpause: z
    .boolean()
    .nullable().optional()
    .describe("Whether the unpause was manually set by a user"),
  failure_reason: z
    .string()
    .nullable().optional()
    .describe("Specific reason for failure if status is 'failed'"),
  current_step_id: z
    .string()
    .nullable().optional()
...
export const PhoneNumberSchema = z.object({
  ...
  dnc_other_info: z.union([z.string(), z.object({})]).nullable().optional().describe("Additional DNC information/details"),
...
export const ContactSchema = z.object({
  ...
  emailer_campaign_ids: z.array(z.unknown()).nullable().optional().describe("List of emailer campaign IDs linked to the contact"),
  label_ids: z.array(z.string()).nullable().optional().describe("Array of label IDs attached to the contact"),
  contact_emails: z.array(ContactEmailSchema).nullable().optional().describe("Array of email addresses/objects for the contact"),

  contact_rule_config_statuses: z.array(z.unknown()).nullable().optional().optional().describe("Statuses from contact rule engine configurations"),
  ...
  email_needs_tickling: z.union([z.boolean(), z.string()]).nullable().optional().describe("Email follow-up tickle indicator"), // Docs are conflicting on whether this is a boolean or a string
  ...
  extrapolated_email_confidence: z.union([z.string(), z.number()]).nullable().optional().describe("Confidence score for extrapolated email"), // Docs are conflicting on whether this is a string or a number
  ...
  direct_dial_enrichment_failed_at: z.string().nullable().optional().describe("Timestamp when direct-dial enrichment failed"),
  ...
  // Booleans
  queued_for_crm_push: z.boolean().nullable().optional().describe("Queued to be pushed/synced to CRM (UI default true)"),
  ...
  is_likely_to_engage: z.boolean().nullable().optional().describe("Contact is predicted likely to engage (UI default true)"),
  ...
  phone_numbers: z.array(PhoneNumberSchema).nullable().optional().describe("Phone numbers associated with the contact"),
...
export const AccountSchema = z.object({
  ...
  account_rule_config_statuses: z.array(z.object({
  ...
  })).nullable().optional().describe("Rule configuration statuses"),
...
export const SearchBreadcrumbSchema = z.object({
...
export const BulkEnrichPersonResponseSchema = z.object({
