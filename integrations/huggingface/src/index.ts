...
export default new bp.Integration({
  register: async ({ ctx, client, logger }) => {
    ...
    const modelIds = ctx.configuration.languageModels.split(',').map((el) => el.trim())
    ...
    for await (const id of modelIds) {
      try {
        ...
        if (pipeline_tag !== 'text-generation') {
          logger.forBot().debug(`Model ${id} is not a text generation model. Skipping...`)
          continue
        }
        ...
        logger.forBot().debug(`Error fetching model ${id}: ${e instanceof Error ? e.message : 'Unknown Error'}`)
      }
    }
  ...
  actions: {
    generateContent: async (props): Promise<interfaces.llm.GenerateContentOutput> => {
      ...
      return await generateContent(<interfaces.llm.GenerateContentInput>input, hf, models)
    },
