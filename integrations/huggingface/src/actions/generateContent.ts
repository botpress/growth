...
type HfToolCalls = Awaited<ReturnType<HfInference['chatCompletion']>>['choices'][number]['message']['tool_calls']

function mapToolCalls(hfToolCalls: HfToolCalls): sdk.interfaces.llm.ToolCall[] | undefined {
  return hfToolCalls
...
export async function generateContent(
  ...
  if (!model) {
    throw new InvalidPayloadError(`Model ID "${modelId}" is not allowed by this integration`)
  }
  ...
  const messages = [{ role: 'system', content: input.systemPrompt }, ...input.messages]
  ...
  try {
    ...
    throw new sdk.RuntimeError(`Error calling the model: ${e instanceof Error ? e.message : 'Unknown error'}`)
  }
}
