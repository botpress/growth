...
export class ShopifyClient {
  ...
  public async createWebhook({
    ...
    try {
      let response = await this._client.get(
        `${this._baseUrl}/webhooks.json?topic=${topic}&address=${webhookUrl}`,
      );
  ...
  public async deleteWebhook(webhookId: string, logger: bp.Logger) {
    try {
      const response = await this._client.delete(
        `${this._baseUrl}/webhooks/${webhookId}.json`,
      );
      logger
        .forBot()
        .info(`Shopify webhook: "${webhookId}" deleted successfully`);
  ...
  public async getWebhooks(webhookUrl: string) {
    try {
      const response = await this._client.get(
        `${this._baseUrl}/webhooks.json?address=${webhookUrl}`,
      );
      return response.data;
    } catch (error) {
      throw this._handleError(error);
