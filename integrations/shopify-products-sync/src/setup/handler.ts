...
export const handler: HandlerFunction = async ({
  ...
  try {
    ...
    if (req.headers["x-shopify-topic"] === "products/create") {
      ...
      try {
        await deleteKbArticleById(
          ctx.configuration.knowledgeBaseId,
          body.id,
          client,
        );
        ...
        logger
          .forBot()
          .error(`Error creating new row for product ${product.id}: ${error}`);
      }
      ...
      try {
        ...
        logger
          .forBot()
          .error(
            `Error creating productCreated event for product ${product.id}: ${error}`,
          );
      }
    }
    ...
    if (req.headers["x-shopify-topic"] === "products/delete") {
      ...
      try {
        ...
        logger
          .forBot()
          .error(`Error deleting row for product ${payload.id}: ${error}`);
      }
      ...
      try {
        ...
        logger
          .forBot()
          .error(
            `Error creating productDeleted event for product ${payload.id}: ${error}`,
          );
      }
    }
    ...
    if (req.headers["x-shopify-topic"] === "products/update") {
      ...
      try {
        ...
        logger
          .forBot()
          .error(
            `Error creating productUpdated event for product ${product.id}: ${error}`,
          );
      }
    }
  } catch (error) {
