...
async function syncKb(
  ...
  logger
    .forBot()
    .info(
      "Syncing Shopify products to BP KB for shop:",
      ctx.configuration.shopDomain,
    );
  ...
  logger
    .forBot()
    .info("KB sync complete for shop:", ctx.configuration.shopDomain);
}
...
async function setupWebhooks(
  ...
  logger
    .forBot()
    .info(
      `Setting up webhooks for shop: ${ctx.configuration.shopDomain}, bot: ${ctx.botId}`,
    );
  await Promise.all(
    SHOPIFY_WEBHOOK_TOPICS.map(async (topic) => {
      try {
        ...
        logger
          .forBot()
          .info(
            `Webhook for topic "${topic}" created for shop: ${ctx.configuration.shopDomain}, bot: ${ctx.botId}`,
          );
      } catch (e) {
        logger
          .forBot()
