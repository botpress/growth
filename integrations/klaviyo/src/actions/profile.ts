...
export const updateProfile: bp.IntegrationProps['actions']['updateProfile'] = async ({ ctx, logger, input }) => {
  ...
  try {
    ...
    logger
      .forBot()
      .debug(`Successfully updated profile with ID: ${profileId}. Returned data:`, JSON.stringify(returnValue, null, 2))
...
export const getProfiles: bp.IntegrationProps['actions']['getProfiles'] = async ({ ctx, logger, input }) => {
  ...
  try {
    ...
    const options: Parameters<ReturnType<typeof getProfilesApi>['getProfiles']>[0] = {}
    ...
    logger
      .forBot()
      .debug(`Retrieved ${profiles.length} profiles from Klaviyo. Returned data:`, JSON.stringify(returnValue, null, 2))
...
export const subscribeProfiles: bp.IntegrationProps['actions']['subscribeProfiles'] = async ({
  ...
  try {
    ...
    const subscriptionProfilesData = profileSubscriptions.map((profileSubscription) => {
