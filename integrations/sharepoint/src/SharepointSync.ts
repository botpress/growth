...
export class SharepointSync {
  ...
  private isFileSupported(spPath: string): boolean {
    ...
    if (!SUPPORTED_FILE_EXTENSIONS.includes(fileType)) {
      this.logWarning(
        `File "${spPath}" with type "${fileType}" is not supported. Skipping file.`,
      );
      return false;
    }
    return true;
  ...
  private getOrCreateKB(kbId: string): BotpressKB {
    if (!this.kbInstances.has(kbId)) {
      ...
      this.log(
        `Created BotpressKB instance for KB ${kbId}${this.enableVision ? " with vision enabled" : ""}`,
      );
    }
  ...
  async syncSharepointDocumentLibraryAndBotpressKB(
    oldToken: string,
  ): Promise<string> {
      ...
      switch (ch.ChangeType) {
        ...
        case 1: {
          ...
          for (const kbId of kbIds) {
            await this.getOrCreateKB(kbId).addFile(
              ch.ItemId.toString(),
              relPath,
              content,
            );
        ...
        case 2: {
          ...
          const content = await this.sharepointClient.downloadFile(spPath);
          for (const kbId of kbIds) {
            await this.getOrCreateKB(kbId).updateFile(
              ch.ItemId.toString(),
              relPath,
              content,
            );
        ...
        case 3: {
          ...
          if (res.files.length === 0) {
            this.logger
              .forBot()
              .debug(`[SP Sync] spId=${fileId} not found in any KB`);
            break;
          }
          ...
          res.files.forEach((f) =>
            this.logger
              .forBot()
              .info(`[BP KB] Delete → ${f.key}  (spId=${fileId})`),
          );
          break;
        }
      }
