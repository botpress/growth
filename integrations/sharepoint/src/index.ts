  ...
  register: async ({ ctx, webhookUrl, client, logger }) => {
    ...
    for (const lib of libs) {
      try {
        ...
        logger.forBot().error(`[Registration] (${lib}) Failed to register: ${error instanceof Error ? error.message : String(error)}. Skipping this library.`);
        continue;
      }
    }
  ...
  unregister: async ({ client, ctx, logger }) => {
    ...
    for (const [lib, { webhookSubscriptionId }] of Object.entries(
      ...
      try {
        ...
        logger.forBot().error(`[Unregister] (${lib}) Failed to unregister: ${error instanceof Error ? error.message : String(error)}. Continuing with other libraries.`);
        continue;
      }
    }
  },
